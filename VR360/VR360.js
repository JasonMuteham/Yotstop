...
(function() {
  ...
  var scenes = APP_DATA.scenes.map(function(data) {
    var source = Marzipano.ImageUrlSource.fromString(
      ...
      {
        cubeMapPreviewUrl: APP_DATA.settings.tilePath + data.id + "/preview.jpg"
      }
    );
    var geometry = new Marzipano.CubeGeometry(data.levels);

    var limiter;
    if (APP_DATA.settings.limit) {
        var limitsPos = APP_DATA.settings.limit[1];
        var limitsneg = APP_DATA.settings.limit[0];
        console.log(limitsneg,limitsPos);
        limiter = Marzipano.util.compose(
            Marzipano.RectilinearView.limit.traditional(data.faceSize, 100*Math.PI/180, 120*Math.PI/180),
            Marzipano.RectilinearView.limit.yaw(limitsneg, limitsPos)
          );
    } else {
